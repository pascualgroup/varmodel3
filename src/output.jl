...
function initialize_database()
    ...
    VarModelDB(
        ...
        make_insert_statement(db, "sampled_hosts", 8),
...
function write_targets(db, t, s, threshold, hosts_GI_impact, PCR_sensitivity_level, sampled_hosts)
    ...
    distGeneCount_sort.cumprob = cumsum(distGeneCount_sort.Prob)
    ...
    distGeneCountGroupA_sort.cumprob = cumsum(distGeneCountGroupA_sort.Prob)
...
function write_host_samples(db, t, s, sampled_hosts)
    ...
    for host in sampled_hosts
        ...
        (
            ...
            length(host.liver_infections), length(host.active_infections), Int64(host.n_cleared_infections), Int64(host.generalized_immunity)
        )
        )
