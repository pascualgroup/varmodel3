...
def CalculPTS(inputfile, time):
    if os.path.exists(inputfile):
        ...
        if df['time'].isin([time]).any():
            ...
            if len(df_time['host_id'].unique()) > 1000:
                select = np.random.choice(df_time['host_id'].unique(), size = 1000)
                df_time = df_time[df_time['host_id'].isin(select)]
            ...
            out = out.rename(columns = host_id)
            out.index = host_id
